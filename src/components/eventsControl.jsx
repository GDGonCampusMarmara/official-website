        ...
        {FILTER_BUTTONS.map((btn) => {
          ...
          return (
            <button
              ...
              className={`rounded-full py-2 px-[18px] text-[13px] cursor-pointer tracking-wide transition-all duration-250 border ${isActive ? `${fc.activeBtn} font-medium shadow-lg shadow-[#f9ab00]/10` : `bg-transparent border-white/10 text-[#f8f6f1]/45 ${fc.hoverBtn}`}`}
            >
              {btn.label}
            </button>
          );
